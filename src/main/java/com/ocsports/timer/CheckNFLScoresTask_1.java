...
public class CheckNFLScoresTask_1 extends TimerTask implements ITimerTask {
    private static final String URL = "http://m.espn.go.com/nfl/scoreboard";
    ...
    public void run(boolean ignoreTimes) {
        try {
            ...
            if(ignoreTimes || (currentHour >= 12 && currentHour <= 22)) {
                addTaskMessage("Starting Task " + CLASS_NAME);
                getScores();
                addTaskMessage("Task " + CLASS_NAME + " completed.");
            }
        }
