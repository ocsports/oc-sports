...
<table cellspacing="0" cellpadding="3" border="0" class="league">
    ...
    while( iter.hasNext() ) {
        ...
        <tr class="<%=trClass%>" style="<%=trStyle%>">
            ...
            <td nowrap="true" style="font-weight:bold"><%=lsm.getWins() + "-" + lsm.getLosses() + "-" + lsm.getTies()%></td>
            <%
            ...
            while( iter2.hasNext() ) {
                ...
                if(usx != null && ssm.getEndDate() <= currentTime) {
                    if(usx.getLock() <= 0) {
                        ...
                        if(usx.getLockStatus() == Status.LOCK_STATUS_WON) lockStyle = "font-weight:normal; color:darkgreen";
                        if(usx.getLockStatus() == Status.LOCK_STATUS_LOST) lockStyle = "font-weight:normal; color:red; text-decoration:line-through";
                        if(usx.getLockStatus() == Status.LOCK_STATUS_TIE) lockStyle = "font-weight:normal; color:blue";
